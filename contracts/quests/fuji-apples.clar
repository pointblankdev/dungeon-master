...
(define-public (add-liquidity (amount uint))
    (let
        ...
        ;; (try! (is-unlocked))
        (try! (contract-call? .liquid-staked-charisma transfer amount-a tx-sender contract none))
        (try! (contract-call? .liquid-staked-charisma transfer amount-b tx-sender contract none))
...
(define-public (remove-liquidity (amount uint))
    (let
        ...
        (try! (as-contract (contract-call? .liquid-staked-charisma transfer amount-a contract sender none)))
        (try! (as-contract (contract-call? .liquid-staked-charisma transfer amount-b contract sender none)))
        (var-set block-counter (+ (var-get block-counter) (var-get blocks-per-tx)))
        (ok true)
    )
)
...
(ft-mint? index-token u1000000000 .iron-forge)

(ft-mint? index-token u10000000000 .battle-royale)