...
(define-public (set-complete (address principal) (quest-id uint) (state bool))
	(begin
		(try! (is-oracle))
	    ;; (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.dme006-quest-completion set-complete (address principal) (quest-id uint) (state bool))
	    (contract-call? .dme006-quest-completion set-complete address quest-id state)
	)
)
