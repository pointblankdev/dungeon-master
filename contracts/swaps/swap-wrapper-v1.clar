...
(define-constant err-spend-too-high (err u4002))
...
(define-public (swap-stx-for-fenrir (amount-stx uint) (amount-fenrir uint) (amount-swelsh uint) (amount-sodin uint) (amt-in-max uint))
    (let (
        ...
        (print {fee-percent: fee-percent, amount-swelsh-plus-fees: amount-swelsh-plus-fees, amount-sodin-plus-fees: amount-sodin-plus-fees})
        (try! (stx-transfer? amount-stx sender contract))
        (try! (as-contract (contract-call? .univ2-router swap-tokens-for-exact-tokens u26 .wstx .liquid-staked-welsh-v2 .wstx .liquid-staked-welsh-v2 .univ2-share-fee-to amt-in-max amount-swelsh-plus-fees)))
        (try! (as-contract (contract-call? .univ2-router swap-tokens-for-exact-tokens u24 .wstx .liquid-staked-odin .wstx .liquid-staked-odin .univ2-share-fee-to amt-in-max amount-sodin-plus-fees)))
...
(define-public (swap-fenrir-for-stx (amount-fenrir uint) (amt-out-min uint))
    (let (
        ...
        (try! (as-contract (contract-call? .univ2-router swap-exact-tokens-for-tokens u26 .wstx .liquid-staked-welsh-v2 .liquid-staked-welsh-v2 .wstx .univ2-share-fee-to (unwrap-panic (contract-call? .liquid-staked-welsh-v2 get-balance contract)) amt-out-min)))
        (try! (as-contract (contract-call? .univ2-router swap-exact-tokens-for-tokens u24 .wstx .liquid-staked-odin .liquid-staked-odin .wstx .univ2-share-fee-to (unwrap-panic (contract-call? .liquid-staked-odin get-balance contract)) amt-out-min)))
