...
(define-public (unstake (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? .welshcorgicoin-token transfer amount-welsh contract sender none)))
		)
		(ok true)
	)
)
...
(define-private (total-welsh-in-pool)
	(unwrap! (contract-call? .welshcorgicoin-token get-balance contract) u1)
)
