...
(define-constant exchange-rate u238)
(define-constant ONE_6 (pow u10 u6)) ;; 6 decimal places

(define-fungible-token dingo)

(define-data-var token-name (string-ascii 32) "Dingo")
(define-data-var token-symbol (string-ascii 10) "DINGO")
(define-data-var token-uri (optional (string-utf8 256)) (some u"https://charisma.rocks/dingo.json"))
...
(define-public (mint (amount uint))
	(begin
		(let
			...
			(try! (contract-call? .welshcorgicoin-token transfer (* amount exchange-rate) sender (as-contract tx-sender) none))
			(try! (contract-call? .roo-token transfer amount sender (as-contract tx-sender) none))
			(try! (ft-mint? dingo amount sender))
		)
		(ok true)
	)
)
...
(define-public (burn (amount uint))
	(begin
		(let
			...
			(try! (as-contract (contract-call? .welshcorgicoin-token transfer (* amount exchange-rate) tx-sender sender none)))
			(try! (as-contract (contract-call? .roo-token transfer amount tx-sender sender none)))
			(try! (ft-burn? dingo amount sender))
		)
		(ok true)
	)
)
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
	(begin
		...
		(ft-transfer? dingo amount sender recipient)
	)
)
...
(define-read-only (get-balance (who principal))
	(ok (ft-get-balance dingo who))
)

(define-read-only (get-total-supply)
	(ok (ft-get-supply dingo))
)
