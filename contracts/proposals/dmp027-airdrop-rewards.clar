;; Title: DMP027 - Airdrop Rewards
;; Author: rozar.btc
;; Synopsis:
;; This proposal airdrops rewards to early users to account for the supply increase.

(impl-trait .dao-traits-v2.proposal-trait)

(define-constant err-generic (err u500))

(define-constant drip-increase-block u149340)
...
(define-public (execute (sender principal))
	(begin
		...
		;; update decimals points for the Charisma token from 0 -> 6
		(try! (contract-call? 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token set-decimals u6))
		;; broadcast the token metadata update
		(print {
			notification: "token-metadata-update",
			payload: {
				contract-id: 'SP2D5BGGJ956A635JG7CJQ59FTRFRB0893514EZPJ.dme000-governance-token,
				token-class: "ft"
			}
		})
		;; update faucet drip amount
		(try! (contract-call? .dme005-token-faucet-v0 set-drip-amount u5000000))
        (ok true)
	)
)
